...
<?php
	 foreach($all->fetchAll() as $row) {
		 $pretty_user = "<div class='panel-heading' style='color: " . htmlspecialchars($row[3]) . "'>" . htmlspecialchars($row[1]) . "</div>";
		 echo ('<div class="panel panel-default">' . $pretty_user . '<div class="panel-body">' . htmlspecialchars($row[2]) . '</div></div>');
	}
